...
// Based on https://www.mouser.com/pdfdocs/DFR0464Datasheet.pdf
// And built to be suitable for lcd_1602_i2c
...
// Used to help detect nibble bytes that are sent to lcd_1602
static struct 
{
    uint8_t high;
...
// Slave handler for lcd_1602
static void lcd_1602_i2c_slave_handler(i2c_inst_t *i2c, i2c_slave_event_t event){
    switch(event){
        uint8_t data; 
        uint8_t current;
        case I2C_SLAVE_RECEIVE: // For when the master writes to lcd_1602
            current = i2c_read_byte_raw(i2c);
            // The reason for such complex logic in receiving
            // is due to the fact that lcd_1602 receives data 
            // in nibble bytes, which is a byte split up into 
            // 6 other bytes to be read by the actual lcd_1602

            // Ensures that the LSB of the actual byte is filled
            if (nibbleBytes.high_enable_pins_1 && nibbleBytes.high_enable_pins_2){
                ...
                // Combines the msb and lsb of the received data to form the full byte
                data = ((nibbleBytes.high & 0xF0) | (nibbleBytes.low >> 4));
                printf("Data received: %02X\n\n", data);

                // Checks if the address has been written for cgram or ddram
                if (lcdMemoryMap.cgram_address_written){
                    ...
                    // Handles all the commands available in lcd_1602
            ...
            break;
        case I2C_SLAVE_REQUEST: // THere are no features in lcd_1602 that involve writing
            break;
...
// Used by master to write a byte to lcd_1602
...
// Goes to location on LCD
void lcd_set_cursor(int line, int position) {
    int val = (line == 0) ? 0x80 + position : 0xC0 + position;
    lcd_send_byte(val, LCD_COMMAND);
}

// Sends char to lcd_1602
static void inline lcd_char(char val) {
    lcd_send_byte(val, LCD_CHARACTER);
}

// Sends string to lcd_1602
...
// Initialises lcd_1602 for the master 
void lcd_init() {
...
// Runs the master to interact with lcd_1602
...
// Sets up lcd_1602 slave emulator
void setup_slave_lcd1602(){
...
void i2c_slave_init_Caller_lcd1602(){
