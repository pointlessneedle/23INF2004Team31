...
void list_and_read_all_files(const char *path) {
    ...
    while (true) {
        ...
        if (filinfo.fattrib & AM_DIR) {
            ...
            // Print statements on serial her
            printf("File: %s\r\n", filinfo.fname);
...
// Function to append to existing file in microsdcard
...
// Function to get the system's time using tm* structure that defines time
int day, month, year;
...
// Function to get value of a key in the map. int 1 = Winbond, 2 = Micron. This will be used in the read_chip_id function
...
// Functiont to check if the program is doing something and is busy. If busy, program will have to wait.
// Check is conducted with the BUSY_STATUS flag defined above
...
// Function to read from the connected flash
...
// Main function to read the ID of flash chip, saving the content to strings and sending to a structure defined above.
// Program includes checking both Winbond and Micron products known/defined in the list above
FlashID __not_in_flash_func(read_chip_id)(spi_inst_t *spi, uint cs_pin) {
    ...
    // Getting manufacturerID and deviceID. Received replies from rx_buffer will be saved into different buffers
    ...
    // Combining buffer2 and buffer3 together to get the deviceID
    uint16_t device_id = ((uint16_t)buffer2 << 8) | buffer3;

    // Check if buffer4 matches Winbond ManufacturerID as 0xEF
    if (buffer1 & 0xEF){
    ...
    // Check if buffer1 matches Micron ManufactuerID as 0x20
...
int exec_flashmicro() {
    ...
    // Printing and saving content information to the file in sdcard
